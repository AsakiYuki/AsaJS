...
import { AnimationProperties } from "../../types/ui/properties/element/Animation.js"
...
export class Animation<T extends AnimType> {
	protected keyframes: AnimationKeyframe<AnimType>[] = []
	...
	private addKeyframes(...keyframes: Anim<T>[]) {
		if (this.smartAnimationMode === "none") {
			...
			for (const $ of keyframes) {
				...
				const keyframeController = new AnimationKeyframe(type, <AnimationProperties<T>>properties)
				const prevKeyframe = this.lastKey()
				if (prevKeyframe) prevKeyframe.setNext(keyframeController)
				this.keyframes.push(keyframeController)
			}
	...
	setLoop(boolean: boolean) {
		...
		if (this.loop) {
			const prevKeyframe = this.lastKey()
			if (prevKeyframe) prevKeyframe.setNext(this.firstKey() as AnimationKeyframe<T>)
		} else {
			const prevKeyframe = this.lastKey()
